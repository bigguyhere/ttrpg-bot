...
export class GameInterpreter extends Interpreter {
    ...
    constructor(
        ...
        this.gameName = UtilityFunctions.formatNullString(
            options.getString("game-name"),
            / /g,
            "_"
        );
    }
    ...
    public createGame(): string {
        const gameType = UtilityFunctions.formatNullString(
            this.options.getString("game-type")
        );
    ...
    public async help(): Promise<string | null> {
        const commandName = UtilityFunctions.formatNullString(
            this.options.getString("command-name"),
            / /g,
            "-"
        );
        let commandStr = "";
        const commands =
            await this.interaction.client.application.commands.fetch({
