...
export class InitInterpreter extends Interpreter{
    ...
    public async next(activeGame : ActiveGame, client : Client<boolean>) : Promise<string>{
            ...
            let nextInit = await Initiative.nextTurn(this.gamedb, this.tableNameBase, activeGame)
    ...
    public async addCharacter(activeGame : ActiveGame, bridge : Bridge) : Promise<string>{
        ...
        let chrName = UtilityFunctions.formatString(this.options.getString('char-name', true))
