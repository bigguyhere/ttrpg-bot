...
export class InventoryInterpreter extends Interpreter {
    ...
    public async modify(chrName: string, bridge: Bridge): Promise<string> {
        ...
        const item = UtilityFunctions.formatString(
            this.options.getString("item-name", true)
        );
        const quant = this.options.getNumber("quantity");
        const desc = UtilityFunctions.formatNullString(
            this.options.getString("description")
        );
        const weight = this.options.getNumber("weight");

        const inv = await Inventory.getItem(
            this.gamedb,
            this.tableNameBase,
            chr.id,
            item
    ...
    public async view(
        ...
        const itemName = UtilityFunctions.formatNullString(
            this.options.getString("item-name")
        );

        if (itemName == null) {
            const chrItems = await chr.getAllChrItems(
                this.gamedb,
                this.tableNameBase
            );

            const embeds = await chr.buildInventoryEmbed(
            ...
            const embedBuilder = await item.buildViewEmbed(
                this.interaction.user,
