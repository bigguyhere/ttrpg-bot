...
module CommandBridge {
    ...
    async function bridge(
        ...
        const gameName = UtilityFunctions.formatNullString(
            options.getString("game-name"),
            / /g,
            "_"
        );

        const activeGame = await ActiveGame.getCurrentGame(
            gamedb,
            "GamesDB",
            guildID,
            gameName
        );
        const tableNameBase = `${guildID}_${
            activeGame?.gameName == null ? gameName : activeGame?.gameName
        }`;
        const bridge = SelectBridge.select(
            activeGame?.gameType,
            gamedb,
            tableNameBase
        );
        ...
        if (subcommandName != null) {
            const disabledCmd = bridge.getDisabledCmd(
                commandName,
                subcommandName
            );
        ...
        if (commandName === "game") {
            const gameInterpreter = new GameInterpreter(
                gamedb,
                tableNameBase,
                options,
                client,
            ...
            switch (subcommandName) {
                ...
                case "view":
                    ...
                    return await invInterpreter.view(
                        chrName,
                        bridge,
                        activeGame
                    );
            ...
            const initInterpreter = new InitInterpreter(
                gamedb,
                tableNameBase,
                options,
                client,
            ...
            switch (subcommandName) {
                ...
                case "add":
                    return await initInterpreter.addCharacter(
                        activeGame,
                        bridge
                    );
                case "remove":
        ...
        const retVal = await bridge.parse(
            commandName,
            subcommandName,
            options,
            activeGame,
