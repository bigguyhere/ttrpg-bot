...
export class SkillInterpreter extends Interpreter {
    ...
    private async viewChrTBs(chrName : string, isDM : boolean, isDynamic : boolean, activeGame : ActiveGame): Promise<string | null>{
        const chr = await DRCharacter.getCharacter(this.gamedb, this.tableNameBase, chrName)
        if(chr == null){
            return `Error finding character ${chrName}.`
        } 
        ...
        const embeds = isDynamic 
                    ? await DRSkill.buildDynamicViewEmbed(this.interaction.user, this.interaction.guild, this.client, activeGame, chrSkills)
                    : await chr.buildSkillEmbed(this.interaction.user, this.client, chrSkills);
    ...
    private async viewSkill(skillName : string, isDM : boolean, activeGame : ActiveGame): Promise<string | null>{
        const skill = await DRSkill.getSkill(this.gamedb, this.tableNameBase, skillName)
        if(skill == null){
            return `Error finding skill ${skillName}.`
        } 
    ...
    private async viewTBSummary(isDM : boolean, isDynamic : boolean, activeGame : ActiveGame): Promise<string | null>{
            ...
            const embeds = isDynamic 
                        ? await DRSkill.buildDynamicViewEmbed(this.interaction.user, this.interaction.guild, this.client, activeGame, allSkills)
                        : await DRSkill.buildSummaryEmbed(this.interaction.user, this.interaction.guild, this.client, activeGame, allSkills);
