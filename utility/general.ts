...
module UtilityFunctions {
    ...
    function findMaxMin(
        ...
        const compFunc: Function = findMax
            ? (a: number, b: number) => a > b
            : (a: number, b: number) => a < b; // Finds max or min depending on input

        for (let ind = 0; ind < rolls.length; ++ind) {
            // Instantiate all values in extrema array
            extrema[ind] = false;
        }
        ...
        for (let ind = 0; ind < amnt; ++ind) {
            ...
            for (let rollInd = 0; rollInd < rolls.length; ++rollInd) {
                if (
                    (compFunc(rolls[rollInd], rolls[extInd]) &&
                        !extrema[rollInd]) ||
                    extrema[extInd]
                ) {
                    // If is an extrema and not a duplicate extrema
                    extInd = rollInd;
                }
            }
    ...
    function evaluate(value: string): [string, number] {
        if (value.includes("d")) {
            ...
            if (value.endsWith("adv")) {
                // Equivalent to 2dYt1
                numRolls = 2;
                op = "t";
                opValue = 1;
                value = `1d${diceValue}t2`;
            } else if (value.endsWith("dis")) {
                // Equivalent 2dYb1
                numRolls = 2;
                op = "b";
                opValue = 1;
                ...
                // Explodes dice that meet the value Z by rolling another die with value Y (XdYeZ)
                numRolls = splitArr[0].length === 0 ? 1 : parseInt(splitArr[0]);
                op = "e";
                opValue = parseInt(splitArr[2]);

                errorCheck(
                    diceValue === 1 && opValue === 1,
                    "Cannot roll infinite dice"
                );

                if (isNaN(opValue)) {
                    // Defaults to highest value
                    opValue = diceValue;
                }
            } else {
                numRolls = numRolls =
            ...
            errorCheck(
                opValue > diceValue,
                `Value for **${op}** (*${opValue}*) cannot exceed the diceValue value (*${diceValue}*)`
            );
            ...
            for (let roll = 0; roll < numRolls; ++roll) {
                ...
                if (op == "e" && rollValue == opValue) {
                    // Explodes dice if value to explode is met
                    numRolls++;
                }
            }
            ...
            let extrema: Array<boolean> = [];
            if (op == "t" || op == "b") {
                // Finds extrema for keeping top/bottom amount of dice
            ...
            for (let rollInd = 0; rollInd < rolls.length; ++rollInd) {
                ...
                if (op == "e" && roll == opValue) {
                    // Adds ! for dice that explode
                    strBuilder += "!";
                }

                if ((op == "t" || op == "b") && !extrema[rollInd]) {
                    // Adds strikethrough to rolls not being considered
                    strBuilder = `~~${strBuilder}~~`;
                } else {
                    // Rolls that are being considered are added to total
                    total += roll;
                }

                if (roll == 1 || roll == diceValue) {
                    // If die is min or max value, bold the value
                    strBuilder = `**${strBuilder}**`;
                }
            ...
            if (retStr[retStr.length - 1] == " ") {
                // Removes trailing comma if exists
    ...
    export function infixToPostfix(
        ...
        sections.forEach((sect) => {
            ...
            if (prec !== -1) {
                ...
                while (
                    precedence(peekElem) !== -1 &&
                    prec >= precedence(peekElem)
                ) {
                    // Pop operators until value of equal or higher precedence is found
                ...
                while (op !== undefined && op != "(") {
                    // Pop operators until starting ( is found
                    retArr.push(op);
                    op = opStack.pop();
                }
                retStr += `${sect} `;
    ...
    export function parseRoll(query: string): [string, number] {
        ...
        postfix[0].forEach((elem) => {
            if (typeof elem == "number") {
                // Pushes numerical operands onto the stack
                operandStack.push(elem);
            } else {
                // When an operator string appears, pops two operands and performs the operator's function on them
                ...
                switch (
                    elem // Pushes result of operation back into operand stack for further computation
                ) {
                    case "+":
    ...
    export function getEmoteDisplay(
        client: Client,
        emote: string | null
    ): string {
        let emoteStr;
    ...
    export function formatString(
        baseStr: string | null,
        findPattern: string | RegExp = /"|’|'/g,
        replacerChr: string = ""
    ): string {
        return baseStr == null
            ? "null"
            : baseStr.trim().replace(findPattern, replacerChr);
    }

    export function formatNullString(
        baseStr: string | null,
        findPattern: string | RegExp = /"|’|'/g,
        replacerChr: string = ""
    ): string | null {
    ...
    export async function getMessage(
        ...
        return await channel.messages.fetch(msgID).catch((e) => {
            return undefined;
        });
    }
