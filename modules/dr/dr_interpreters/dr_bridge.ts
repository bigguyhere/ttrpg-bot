...
import { DRUtilityFunctions } from "../dr_utility/dr_utility";
...
export class DRBridge extends Bridge {
    ...
    async getCharacter(char_name: string): Promise<DRCharacter | null> {
        return await DRCharacter.getCharacter(
            this.gamedb,
            this.tableNameBase,
            char_name
        );
    ...
    async parse(
        ...
        if (commandName === "dr-character") {
            const drCharInterpreter = new DRCharacterInterpreter(
                this.gamedb,
                this.tableNameBase,
                options,
                client,
            ...
            const rsInterpreter = new RelationshipInterpreter(
                this.gamedb,
                this.tableNameBase,
                options,
                client,
            ...
            const charName1 = UtilityFunctions.formatString(
                options.getString("character-1", true)
            );
            const charName2 = UtilityFunctions.formatString(
                options.getString("character-2", true)
            );
            switch (subcommandName) {
                case "view":
            ...
            const skillInterpreter = new SkillInterpreter(
                this.gamedb,
                this.tableNameBase,
                options,
                client,
            ...
            const tbInterpreter = new TBInterpreter(
                this.gamedb,
                this.tableNameBase,
                options,
                client,
            ...
            const trialInterpreter = new TrialInterpreter(
                this.gamedb,
                this.tableNameBase,
                options,
                client,
            ...
            switch (subcommandName) {
                ...
                case "add-character":
                    return await trialInterpreter.addCharacter(
                        activeGame,
                        this
                    );
                case "next":
            ...
            const witnesses = UtilityFunctions.parseMultStr(
                UtilityFunctions.formatNullString(
                    options.getString("witnesses")
                )
            );
            const discoverers = UtilityFunctions.parseMultStr(
                UtilityFunctions.formatNullString(
                    options.getString("discoverers", true)
                )
            );
            ...
            witnesses.forEach(async (witness) => {
                new DRCharacter(witness).updateHD(
                    this.gamedb,
                    client,
                    this.tableNameBase,
                    0,
            ...
            discoverers.forEach(async (discoverer) => {
                new DRCharacter(discoverer).updateHD(
                    this.gamedb,
                    client,
                    this.tableNameBase,
                    0,
            ...
            const audioPlayer = await VoiceFunctions.playAudio(
                "media/body-discovery.mp3"
            );
            const member = interaction.member as GuildMember;
            const connection = await VoiceFunctions.getConnection(
            ...
            await VoiceFunctions.setIdleDisconnect(
                audioPlayer,
                connection,
                2000
            );
