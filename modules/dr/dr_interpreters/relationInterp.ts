...
export class RelationshipInterpreter extends Interpreter {
    public async change(charName1: string, charName2: string): Promise<string> {
        ...
        let char1 = await DRCharacter.getCharacter(
            this.gamedb,
            this.tableNameBase,
            charName1
        );
        let char2 = await DRCharacter.getCharacter(
            this.gamedb,
            this.tableNameBase,
            charName2
        );
        ...
        let relationship = await new DRRelationship(
            char1,
            char2
        ).getRelationship(this.gamedb, this.tableNameBase);
    ...
    public async view(charName1: string, charName2: string): Promise<string> {
        ...
        await this.interaction.channel?.send({
            embeds: [
                await relationship.buildViewEmbed(
