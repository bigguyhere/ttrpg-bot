...
import { DRTruthBullet } from "../drmodels/drtruthbullet";
import { DRVote } from "../drmodels/drvote";
import { Initiative } from "../../../models/initiative";
import { UtilityFunctions } from "../../../utility/general";
import { Pagination } from "../../../utility/pagination";
import { Bridge } from "../../../interpreters/abstract_models";
import { InitInterpreter } from "../../../interpreters/initInterp";
...
export class TrialInterpreter extends InitInterpreter {
    public async begin(activeGame: ActiveGame): Promise<string> {
        if (activeGame.messageID != null) {
            return (
                "Cannot start trial as there is already one in progress." +
                " Please end other trial before starting a new one."
            );
        }

        const addAll = this.options.getBoolean("add-all");
        const blackened = UtilityFunctions.formatString(
            this.options.getString("blackened", true)
        );
        const vicitms = UtilityFunctions.parseMultStr(
            UtilityFunctions.formatString(
                this.options.getString("victims", true)
            )
        );
        ...
        const killer = await DRCharacter.getCharacter(
            this.gamedb,
            this.tableNameBase,
            blackened
        );
        ...
        const changes = await DRRelationship.getHDChange(
            this.gamedb,
            this.tableNameBase,
            "Victim"
        );
        ...
        let retStr = await this.initSetup(
            activeGame,
            "2d6",
            true,
            "The Class Trial"
    ...
    public async end(activeGame: ActiveGame): Promise<string> {
        ...
        let chrs = await DRCharacter.getAllCharacters(
            this.gamedb,
            this.tableNameBase,
            true
        );
        ...
        const chrNames = [
            UtilityFunctions.formatNullString(
                this.options.getString("cs-char1")
            ),
            UtilityFunctions.formatNullString(
                this.options.getString("cs-char2")
            ),
        ];
        ...
        const csChrs = chrs.filter((chr) => {
            return chr.name === chrNames[0] || chr.name === chrNames[1];
        });
    ...
    public async addCharacter(
        ...
        if (
            !(await new Initiative(chrName, result[1], emote, user).addToTable(
                this.gamedb,
                this.tableNameBase
            ))
        ) {
    ...
    public async vote(activeGame: ActiveGame): Promise<string | null> {
        ...
        const voterName = UtilityFunctions.formatString(
            this.options.getString("voter-char", true)
        );
        const voteName = UtilityFunctions.formatString(
            this.options.getString("vote", true)
        );

        const voter = await DRCharacter.getCharacter(
            this.gamedb,
            this.tableNameBase,
            voterName
        );
        const vote = await DRCharacter.getCharacter(
            this.gamedb,
            this.tableNameBase,
            voteName
        );
        ...
        const remainingVotes = await DRVote.countRemainingVotes(
            this.gamedb,
            this.tableNameBase
        );
        ...
        if (remainingVotes == 0) {
            ...
            let results = await DRVote.getResults(
                this.gamedb,
                this.tableNameBase
            );
            ...
            const nameStr =
                blackened[0].talent == null
                    ? blackened[0].name
                    : `The ${blackened[0].talent}: ${blackened[0].name}`;
            const replyStr = `All votes counted ! The blackened was chosen to be **${nameStr}** with **${blackened[1]}** votes !`;

            DRCharacter.setToDead(
                this.gamedb,
                this.tableNameBase,
                blackened[0].status !== "Blackened"
            );
            ...
            if (activeGame.channelID != null && activeGame.messageID != null) {
                ...
                const embeds = DRVote.buildSummaryEmbed(
                    this.client,
                    this.interaction.guild,
                    results
                );
    ...
    public async hangman(activeGame: ActiveGame): Promise<string> {
        ...
        const word = UtilityFunctions.formatString(
            this.options.getString("word", true)
        );

        (await this.client.users.fetch(chr.owner)).send(
            `**Hangman\'s Gambit Begin !**\nYour word is: __*${UtilityFunctions.scrambleString(
                word
            ).toUpperCase()}*__`
        );
    ...
    public async interrupt(activeGame: ActiveGame): Promise<string> {
        ...
        const consentGifs = [
            "https://tenor.com/view/consent-makoto-danganronpa-gif-25163640",
            "https://tenor.com/view/consent-gif-20460974",
            "https://tenor.com/view/danganronpa-v3-consent-gif-19290276",
        ];
        const counterGifs = [
            "https://tenor.com/view/makoto-naegi-danganronpa-counter-gif-25163631",
        ...
        const rebuttalGifs = [
            "https://tenor.com/view/rebuttal-showdown-danganronpa-gundham-tanaka-gif-21093184",
        ...
        if (type != null) {
            if (type == 0) {
                ...
                gif =
                    consentGifs[
                        UtilityFunctions.getRandomNum(consentGifs.length) - 1
                    ];
            } else if (type == 1) {
                retStr += "counters !";
                gif =
                    counterGifs[
                        UtilityFunctions.getRandomNum(counterGifs.length) - 1
                    ];
            } else if (type == 2) {
                retStr += "initaites a rebuttal showdown !";
                gif =
                    rebuttalGifs[
                        UtilityFunctions.getRandomNum(rebuttalGifs.length) - 1
                    ];
