...
export class TBInterpreter extends Interpreter {
    ...
    public async view(activeGame : ActiveGame) : Promise<string | null> {    
        ...
        if(chrName != null && tbName != null){
            ...
            return await this.viewChrTBs(chrName, isDM, trialNum, isDynamic, activeGame);
        } else if(tbName != null){
            ...
            return await this.viewTBSummary(isDM, trialNum, isDynamic, activeGame);
        } 
    }

    private async viewChrTBs(chrName : string, isDM : boolean, trialNum : number | null, isDynamic : boolean, activeGame : ActiveGame): Promise<string | null>{
        const chr = await DRCharacter.getCharacter(this.gamedb, this.tableNameBase, chrName);
        if(chr == null){
            return `Error finding character ${chrName}.`;
        } 
        ...
        const embeds = isDynamic 
                    ? await DRTruthBullet.buildDynamicViewEmbed(this.interaction.user, this.interaction.guild, this.client, activeGame, chrTBs)
                    : await chr.buildTBEmbed(this.interaction.user, this.client, chrTBs);
    ...
    private async viewTB(tbName : string, isDM : boolean, trialNum : number | null, activeGame : ActiveGame): Promise<string | null>{
    ...
    private async viewTBSummary(isDM : boolean, trialNum: number | null, isDynamic : boolean, activeGame : ActiveGame): Promise<string | null>{
        ...
        let embeds = isDynamic 
                    ? await DRTruthBullet.buildDynamicViewEmbed(this.interaction.user, this.interaction.guild, this.client, activeGame, allTBs)
                    : await DRTruthBullet.buildSummaryEmbed(this.interaction.user, this.interaction.guild, this.client, activeGame, allTBs);
