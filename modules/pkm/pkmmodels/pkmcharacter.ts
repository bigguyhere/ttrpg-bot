...
export class PokeCharacter extends Character {
    ...
    addToTable(db : mysql.Connection, tableBaseName : string): Promise<boolean>{
            ...
            const queryStr = "Heart, Fitness, Research, Tactics, Advancement, Exp, Money, FirstImp, Calling";
            const valueStr = `${this.heart}, ${this.fitness}, ${this.research}, ${this.tactics}, ${this.advancement}, ${this.exp}, ${this.money}, 
            ${this.firstImpression == null ? "null" : `"${this.firstImpression}"`}, ${this.calling == null ? "null" : `"${this.calling}"`}`;

            return super.addToTable(db, tableBaseName, queryStr, valueStr);
    }

    static getCharacter(db : mysql.Connection, tableBaseName : string, char_name: string): Promise<PokeCharacter | null>{
        ...
        return new Promise((resolve) =>{
            db.query(`SELECT * FROM ${tableBaseName}_Characters WHERE Name = "${char_name}";`, (err, res) =>  {
                ...
                let retChr = new PokeCharacter(res[0].Name, 
                                            res[0].Emote, 
                                            res[0].Pronouns,
                                            res[0].Owner,
                                            res[0].Heart,
    ...
    static getAllCharacters(db : mysql.Connection, tableBaseName : string, onlyAlive : boolean = false): Promise<Array<PokeCharacter> | null>{
        return new Promise((resolve) =>{
            ...
            db.query(`SELECT * FROM ${tableBaseName}_Characters${condStr} ORDER BY Name;`, (err, res) =>  {
                ...
                res.forEach((char: { Name: string; Emote: string | null | undefined; Pronouns: string | null | undefined; 
                    Owner: string | undefined; Heart: number | undefined; Fitness: number | undefined; Research: number | undefined; 
                    Tactics: number | undefined; Advancement: number | undefined; Exp: number | undefined; Money: number | undefined; 
                    FirstImp: string | null | undefined; Calling: string | null | undefined; CHR_ID: number; }) =>{
                let retChr = new PokeCharacter(char.Name, 
                                            char.Emote, 
                                            char.Pronouns,
                                            char.Owner,
                                            char.Heart,
    ...
    async buildViewEmbed(user : DiscordJS.User, client: Client<boolean>): Promise<EmbedBuilder>{
        ...
        .addFields(
            ...
            { name: '\u200B', value: '\u200B' },
            { name: 'Wounds', value: String(this.getCurrentHealth()) , inline: true},
