...
export class PokeDBPKM {
        ...
        if (pkmData) {
            ...
            if (data["types"] && Array.isArray(data["types"])) {
                for (let i = 0; i < data["types"].length; ++i) {
                    ...
                    this.types[i] =
                        Type[typeStr.toUpperCase() as keyof typeof Type];
                }
            }

            if (data["abilities"] && Array.isArray(data["abilities"])) {
                for (let i = 0; i < data["abilities"].length; ++i) {
                    const abilityStr = data["abilities"][i]["ability"][
                        "name"
                    ] as string;
                    this.abilties.push(
                        PkmUtilityFunctions.formatStr(abilityStr)
                    );
                }
            }
    ...
    public static async getPokemonFromSpecies(
    ...
    public async buildViewEmbed(
        ...
        const type1 = Type[this.types[0]],
            type2 = Type[this.types[1]];
        let descStr = `**DM:** ${await client.users.fetch(activeGame.DM)}\n
                        **Type(s):** ${EmojiTypeMap[type1]} ${type1} ${
            EmojiTypeMap[type1]
        }${
            this.types[1] !== Type.NONE
                ? ` - ${EmojiTypeMap[type2]} ${type2} ${EmojiTypeMap[type2]}`
                : ""
        }
                        **Abilities:**`;
        ...
        for (let weakness of weaknesses) {
            descStr += `${EmojiTypeMap[weakness[0]]} ${weakness[0]} (${
                weakness[1]
            }x), `;
        }
