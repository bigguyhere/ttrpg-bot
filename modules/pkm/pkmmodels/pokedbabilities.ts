...
export class PokeDBAbility {
    ...
    public static async getAbilityList(data : any | null) : Promise<PokeDBAbility[]> {
    ...
    public static async buildSummaryEmbed(user : DiscordJS.User, client : Client<boolean>, activeGame : ActiveGame,
         pkm : PokeDBPKM, abilities : Array<PokeDBAbility> | null): Promise<EmbedBuilder[] | null>{
        ...
        for(let i = 0; i < numEmbeds; ++i){
            ...
            embeds[i].setDescription(`**DM:** ${(await client.users.fetch(activeGame.DM))}\n
