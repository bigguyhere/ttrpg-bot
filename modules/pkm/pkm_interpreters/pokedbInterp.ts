...
export class PokeDBInterpreter extends Interpreter {
    ...
    public async viewAbilities(pkmName : string, activeGame : ActiveGame) : Promise<string | null> {
        let form = UtilityFunctions.formatNullString(this.options.getString('form'));
        ...
        let result : AxiosResponse | null = null;
        try{
            result = await axios.post(this.apiURL, PkmUtilityFunctions.getPkmAbilityQuery(
                'moveQuery', 
                pkmName, 
                searchParam));
        ...
        const response = result?.data.data.species[0];
        const pokedb = new PokeDBPKM(pkmName, form, response);
        const abilityList = await PokeDBAbility.getAbilityList(response.pokemon[0]);

        const replyStr = `Pokemon **${pokedb.name}'s** abilities has been successfully viewed.`;
        const embeds = await PokeDBAbility.buildSummaryEmbed(this.interaction.user, this.interaction.client, activeGame, pokedb, abilityList);
    ...
    public async viewAbility(abilityName : string, activeGame : ActiveGame) : Promise<string | null> { 

        UtilityFunctions.errorCheck(!abilityName, 'Name must exist');
        UtilityFunctions.errorCheck(typeof abilityName !== 'string', 'Name must be a string');
        abilityName = UtilityFunctions.formatString(abilityName, /\s/g, '-').toLowerCase();
        UtilityFunctions.errorCheck(abilityName.length === 0, 'Name cannot be empty');
        
        let result : AxiosResponse | null = null;
        try{
            result = await axios.post(this.apiURL, PkmUtilityFunctions.getAbilityQuery('abilityViewQuery', abilityName));
        ...
        const embed = await ability.buildViewEmbed(this.interaction.user, this.interaction.client, activeGame);
